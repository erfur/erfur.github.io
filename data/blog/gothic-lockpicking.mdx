...
- Follow parent links to recover the shortest path. There can be up to `7^N` possible states, so the search grows quickly as layers are added. For the supported maximum of seven layers, however, the state space is still finite and manageable. Sometimes the simple algorithm is the correct one. BFS may not look particularly exotic, but it gives us exactly what this puzzle needs: either a shortest solution or proof that no solution exists.
...
[^1]: [Gothic Lockpick Solver](https://xetoxyc.github.io/gothic-remake-lockpicker/)

[^2]: [Gothic 1: Remake Lock Solver](https://locksolver.com/)

[^3]: [Gothic Lockpick Solver](https://sh1ftry.github.io/gothic-lockpick-solver/)

[^4]: [Lock Picker](https://gothic-lock-picking.com/lock/)
